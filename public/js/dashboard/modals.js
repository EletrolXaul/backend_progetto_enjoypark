...
$(document).ready(function () {
    ...
    $(document).on("click", ".edit-shop", function () {
        ...
        $("#edit_shop_opening_hours").val(shopOpeningHours || ""); // Usa la variabile invece di accedere nuovamente all'attributo
    ...
    $(document).on("click", ".edit-service", function () {
        ...
        console.log("Service data:", {
            ...
            icon: $(this).data("icon"),
            available24h: $(this).data("available-24h"),
            locationX: $(this).data("location-x"),
            locationY: $(this).data("location-y"),
...
$("#saveShopBtn").click(function () {
    ...
    // Assicurati che specialties sia un array JSON valido
    const specialtiesText = $("#addShopForm #specialties").val();

    // Debug: mostra il valore delle specialità prima della conversione
    console.log("Specialties text:", specialtiesText);

    try {
        if (specialtiesText.trim() === "") {
            // Se vuoto, imposta array vuoto
            $("#addShopForm #specialties").val(JSON.stringify([]));
        } else {
            // Prova a parsare come JSON
            const specialtiesArray = JSON.parse(specialtiesText);
            // Se è già un array valido, riassegna formattato
            $("#addShopForm #specialties").val(
                JSON.stringify(specialtiesArray)
            );
        }
    } catch (e) {
        // Se non è un JSON valido, tratta come stringa singola
        const singleSpecialty = specialtiesText.trim();
        if (singleSpecialty !== "") {
            $("#addShopForm #specialties").val(
                JSON.stringify([singleSpecialty])
            );
        } else {
            $("#addShopForm #specialties").val(JSON.stringify([]));
        }
    ...
    shopApi.addShop(
        ...
        function (error) {
            ...
            console.log("Full error response:", error.responseText || error);
    ...
    $("#saveRestaurantBtn")
        ...
        .on("click", function () {
            ...
            console.log(
                ...
                $("#addRestaurantForm #features").val()
            );
